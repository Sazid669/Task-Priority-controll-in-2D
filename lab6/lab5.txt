...
class Configuration2D(Task):
    def __init__(self, name, desired, FFVelocity = None, K = None, link = None):
        super().__init__(name, desired, FFVelocity, K)
        self.J = np.zeros((3,5)) # Initializing with proper dimensions
    ...
    def update(self, robot):
        ...
        R = robot.getEETransform() [:3,:3]
        angle = angle=np.arctan2(R[1,0],R[0,0])
...
class JointLimit(Task):
    def __init__(self, name, joint, safe_set, thresholds):
        super().__init__(name, 0)
        self.J = np.zeros((1,3)) # Initializing with proper dimensions
        self.err = np.zeros((1,1)) # Initializing with proper dimensions
    ...
    def update(self, robot):
        ...
        #Activation update
        if self.active==0 and robot.getJointPos(self.joint) >= self.safe_set[1] - self.thresholds[0]:
            self.active = -1
        elif self.active== 0 and robot.getJointPos(self.joint) <= self.safe_set[0] + self.thresholds[0]:
            self.active = 1
        elif self.active== -1 and robot.getJointPos(self.joint) <= self.safe_set[1] - self.thresholds[1]:
            self.active = 0
        elif self.active==1 and robot.getJointPos(self.joint) >= self.safe_set[0] + self.thresholds[1]:
            self.active = 0
